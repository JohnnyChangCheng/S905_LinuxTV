...
static AVL_Tuner global_tuner_s = 
{
    ...
    DTVMode_DVBSX,
    666*1000*1000,
    36*1000*1000,//IF frequency setting, not used for S tuner
   ...
   if(tuner_t2_isdbt_c_initflag == 0)
    {
        ...
        if(global_tuner_t_c.fpInitializeFunc != NULL)
        {
            ...
            if(return_code!=AVL_EC_OK)
            {
             ...
             printf("[AVL_Init] T/C Tuner Init successed!\n");           
        }
...
{
        AVL_uint32 uiChannelStart = start_khz;//KHz
        AVL_uint32 uiChannelStop = end_khz;//KHz
        AVL_uint32 uiChannelUse = 0;
        int ret = 0;
        for(uiChannelUse = uiChannelStart;uiChannelUse <= uiChannelStop;uiChannelUse = uiChannelUse+bandwidth_khz*1000)
        {
            ret=AVL_ScanChannel_Tx(uiChannelUse, bandwidth_khz*1000); 
        }
        return ret;
}
