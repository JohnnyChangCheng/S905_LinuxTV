...
# if defined(__OS2__) && defined(__INNOTEK_LIBC__)
    ...
    return ( noovo_iconv_t )p_os2_iconv;
# else
    return iconv_open( tocode, fromcode );
# endif
#else
    return (noovo_iconv_t)(-1);
#endif
}
...
#ifndef __linux__
    if ( cd == (noovo_iconv_t)(-2) || cd == (noovo_iconv_t)(-3) )
    {
        unsigned char *out = NULL;
        if (outbuf != NULL)
            out = (unsigned char *)*outbuf;

        const uint16_t *ucs4_table = ( cd == (noovo_iconv_t)(-2) ? ISO6937toUCS4 : ISO6937_2toUCS4 );
