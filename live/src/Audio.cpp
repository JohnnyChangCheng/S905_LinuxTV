...
void Audio::SetTool(AVStream* input_stream,int outputindex ,AVFormatContext* ofmt_ctx,std::shared_ptr<Packet_wrapper> packet_wrapper){
...
{
    /* Packet used for temporary storage. */
    AVPacket* output_packet=(AVPacket*)malloc(sizeof(AVPacket));
    int error;
    /* Set the packet data and size so that it is recognized as being empty. */
    av_new_packet(output_packet,512*1024) ;
