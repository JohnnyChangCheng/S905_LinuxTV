...
void Adinsert::_process(std::shared_ptr<Ofmt_list> fmt_wrapper){
    ...
    while(1){
        ...
        if(_index_type[pkt.stream_index]==AVMEDIA_TYPE_AUDIO){
            pkt.pts = av_rescale_q_rnd(pkt.pts, _ifmt_ctx->streams[i]->time_base, audio_outstream->time_base, AV_ROUND_NEAR_INF);
            pkt.dts = av_rescale_q_rnd(pkt.dts,  _ifmt_ctx->streams[i]->time_base, audio_outstream->time_base, AV_ROUND_NEAR_INF);
            ...
            pkt.pts = av_rescale_q_rnd(pkt.pts, _ifmt_ctx->streams[i]->time_base, video_outstream->time_base, AV_ROUND_NEAR_INF);
            pkt.dts = av_rescale_q_rnd(pkt.dts,  _ifmt_ctx->streams[i]->time_base, video_outstream->time_base, AV_ROUND_NEAR_INF);
